...
async def start_webhook_server(bot) -> None:
    ...
    # Character recommendation system routes
    try:
        from core.recommendations import register_routes as _register_rec_routes
        _register_rec_routes(app, bot)
    except Exception:
    ...
    try:
        from core.legal_page import register_routes as _register_legal_routes
        _register_legal_routes(app)
    except Exception:
        log.exception("Failed to register legal page routes (non-fatal)")
