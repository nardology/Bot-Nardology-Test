...
class SlashOwner(commands.Cog):
    ...
    @owner.command(name="dev_weekly_topics", description="DEV: show the 3 weekly talk topics stored for a user + character")
    @app_commands.describe(
        user="Target Discord user (defaults to you)",
        character_id="Character style_id (defaults to that user's selected character)",
    )
    async def dev_weekly_topics(
        self,
        interaction: discord.Interaction,
    ...
    async def owner_global_quest_link(self, interaction: discord.Interaction):
        ...
        token = generate_token(int(interaction.user.id), "admin")
        url = f"{base}/admin?token={token}&panel=global-quest"
        await _ephemeral(interaction, "Check your DMs for the admin link (Global quest tab).")
        try:
            await interaction.user.send(
                "**Global quest** — open Admin → Global quest tab (valid 30 days):\n" + url + "\n\nPublic status page: " + base + "/global-quest"
            )
        except discord.Forbidden:
            await interaction.followup.send(
